...
// URL publica conocida de R2 (fallback si el backend no expone R2_PUBLIC_URL
// o si aun no resolvio en el primer render).
const FALLBACK_PUBLIC_URL = 'https://pub-cabe26a560384a89a7e2a82367fb1813.r2.dev';

// Cache para la URL publica de R2 (se obtiene del backend)
let cachedPublicUrl: string | null = null;
let publicUrlPromise: Promise<string | null> | null = null;

/**
 * Obtiene la URL publica de R2 desde el backend
 */
async function getPublicUrlFromBackend(): Promise<string | null> {
  ...
  publicUrlPromise = (async () => {
    try {
      ...
      // Si el backend no tiene R2_PUBLIC_URL configurado, usar la URL publica conocida
      // para evitar caer en GET /files/{key} (que falla con keys que contienen "/")
      cachedPublicUrl = data.publicUrl || FALLBACK_PUBLIC_URL;
      return cachedPublicUrl;
    } catch (error) {
      console.error('Error obteniendo URL publica de R2:', error);
...
function extractKeyFromSignedUrl(signedUrl: string): string | null {
  try {
    ...
    if (urlObj.hostname.includes('r2.cloudflarestorage.com')) {
      ...
      // La key es el resto de la ruta despues del bucket
      // En R2, el formato suele ser /[bucket]/[key] o la key es toda la ruta si el bucket esta en el hostname
      if (pathParts.length > 0) {
        // Intentar detectar si el primer segmento es un bucket conocido o si la key empieza por carpetas conocidas
        const firstSegment = pathParts[0];
        const knownFolders = ['photos', 'actas'];
...
/**
 * Convierte una URL firmada de R2 a URL publica permanente
 */
async function convertSignedUrlToPublic(signedUrl: string): Promise<string> {
  ...
  // Si no podemos extraer la key o no hay URL publica, devolver la URL original
  return signedUrl;
}
 ...
 * Si es una key, genera una URL fresca construyendo la URL publica directamente
 * Si es una URL firmada, la convierte a URL publica
 */
export function useFileUrl(keyOrUrl: string | null | undefined): string | null {
  const [url, setUrl] = useState<string | null>(null);
  const [publicUrl, setPublicUrl] = useState<string | null>(null);

  // Obtener la URL publica de R2 una vez al montar el componente
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (keyOrUrl.startsWith('http://') || keyOrUrl.startsWith('https://')) {
      // Si es una URL firmada de R2 (tiene parametros de expiracion), convertirla a publica
      if (keyOrUrl.includes('r2.cloudflarestorage.com')) {
        // Intentar extraer la key y construir URL publica
        const extractedKey = extractKeyFromSignedUrl(keyOrUrl);
        if (extractedKey && publicUrl) {
          // Construir URL publica directamente
          const publicUrlFinal = `${publicUrl}/${extractedKey}`;
          setUrl(publicUrlFinal);
          return;
        }
        // Si no se puede extraer la key, intentar convertir usando la funcion
      ...
      // Si ya es una URL publica de R2 (detecta cualquier .r2.dev), usarla directamente
    ...
    // Si es una key (ruta relativa), construir la URL publica de R2 directamente.
    // Si publicUrl aun no resolvio (primer render), usar el fallback conocido en vez
...
/**
 * Hook para obtener multiples URLs frescas
 */
  ...
  useEffect(() => {
    ...
    const fetchUrls = async () => {
      const urlPromises = keysOrUrls.map(async (keyOrUrl) => {
        ...
        // Si es una URL completa, verificar si necesita conversion
        if (keyOrUrl.startsWith('http://') || keyOrUrl.startsWith('https://')) {
          // Si es una URL firmada de R2, intentar extraer la key y construir URL publica
          if (keyOrUrl.includes('r2.cloudflarestorage.com')) {
            const extractedKey = extractKeyFromSignedUrl(keyOrUrl);
            if (extractedKey && publicUrl) {
              // Construir URL publica directamente
        ...
        // Si es una key (ruta relativa), construir la URL publica de R2 directamente.
        // Si publicUrl aun no resolvio, usar el fallback conocido (evita /files/{key} -> 404).
        const base = publicUrl || FALLBACK_PUBLIC_URL;
