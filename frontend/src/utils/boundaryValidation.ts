...
/**
 * Verifica si un punto esta dentro de un poligono usando el algoritmo ray casting
 */
...
/**
 * Funcion recursiva para verificar si un punto esta en una geometria (Polygon o MultiPolygon)
 */
...
/**
 * Verifica si un punto esta dentro de cualquier poligono de un GeoJSON
 */
...
/**
 * Carga los limites de Candelaria desde los archivos KMZ
 */
export async function loadCandelariaBoundaries(): Promise<GeoJSON | null> {
  try {
    ...
    if (!response.ok) {
      throw new Error('No se pudo cargar los limites');
    }
    ...
    if (!kmlFile) {
      throw new Error('No se encontro archivo KML dentro del KMZ');
    }
    ...
    console.error('Error cargando limites de Candelaria:', error);
    return null;
  }
}

/**
 * Verifica si un punto esta dentro de Candelaria
 */
...
/**
 * Carga los limites de Santa Fe desde los archivos KMZ
 */
export async function loadSantaFeBoundaries(): Promise<GeoJSON | null> {
  try {
    const response = await fetch(`/boundaries/KMZ_Sectores_Catastrales_SF_2026.kmz?t=${Date.now()}`);
    if (!response.ok) {
      throw new Error('No se pudo cargar los limites de Santa Fe');
    }
    ...
    console.error('Error cargando limites de Santa Fe:', error);
    return null;
  }
}
...
/**
 * Carga los limites de barrios desde el archivo KML
 */
...
/**
 * Detecta el barrio de una coordenada, validando primero que caiga dentro de
 * la localidad de Santa Fe y que no este dentro de Candelaria (prohibida).
 * Devuelve null si la coordenada esta fuera de la zona de trabajo o si no
 * se pudo determinar el barrio.
 */
export async function detectarBarrio(lat: number, lng: number): Promise<string | null> {
  const enCandelaria = await isPointInCandelaria(lat, lng);
  if (enCandelaria) return null;

  const limitesSantaFe = await loadSantaFeBoundaries();
  if (limitesSantaFe && !isPointInBoundaries(lat, lng, limitesSantaFe)) {
    return null;
  }

  return findBarrioByPoint(lat, lng);
}
